...
@scheduler.scheduled_job('cron', id='stock_day_inc_collect', minute='45', day_of_week='0-4', hour='15')
def stock_day_inc_collect():
    ...
    stock_time_inc_spider = StockTimeIncSpider(stock_analyzer=None, symbols=symbols, inc_persist=True, hit_persist=True)
...
@scheduler.scheduled_job('cron', id='stock_day_realtime_collect', second='*/60', day_of_week='0-4', hour='9-23', max_instances=1)
def stock_day_realtime_collect():
	...
	symbols = GeodeClient.get_instance().query_all_stock_symbols()
	if not with_gem:
	    symbols = CommonUtils.filter_symbols(symbols)[:20]
	...
	stock_day_inc_spider = StockDayIncSpider(stock_analyzer=stock_day_analyzer, symbols = symbols, inc_persist=False, hit_persist=True)
        ...
        #初始化实时(分时)爬虫器
        global stock_time_inc_spider
        stock_time_inc_spider = StockTimeIncSpider(stock_analyzer=stock_time_analyzer, symbols = symbols, inc_persist=False, hit_persist=True)
...
def stock_time_realtime_collect():
