...
path.append('/home/ubuntu/scripts/dbs')
...
class StockDayHistorySpider:
	...
	count = 0
		...
		for quarter in range(start_quarter, end_quarter + 1):
		    ...
		    #封装昨日收盘价
		    stock_days = self.compose_stock_day_with_last_close(stock_days)

		    #过滤提取2017-10-25
		    target_datestamp = TimeUtils.datestring2timestamp('2017-10-25')
		    stock_days = list(filter(lambda stock_day: stock_day.day == target_datestamp, stock_days))
...
#		    continue	
    ...
    '''	获取last_close，季度临界点不可取	'''
    def compose_stock_day_with_last_close(self, stock_days):
	try:
	    if stock_days is not None and len(stock_days) > 0:
		day_map = {}
		for stock_day in stock_days:
		    day_map[stock_day.day] = stock_day


		for stock_day in stock_days:
		    stock_day.last_close = self.get_last_day_close(day_map, stock_day.day)
    ...
    '''	季度的临界日期不可取	'''
    def get_last_day_close(self, day_map, cur_datestamp):
	days = day_map.keys()
	days = sorted(days)
	for i in range(1, len(days)):
	    if (days[i] == cur_datestamp):
		return day_map[days[i-1]].close
	return None
