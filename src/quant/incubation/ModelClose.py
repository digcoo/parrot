...
#            当前时间段>2:30
	    ...
	    last1_stock_day = BaseStockUtils.pre_stock_day(hist_days, 1, self.today)
            ...
            #大双头大阳线
            is_bdouble_red_head = True
            is_bdouble_red_head = is_bdouble_red_head & (is_red)
            is_bdouble_red_head = is_bdouble_red_head & ((column_shadow > 0.01) & (column_shadow < 0.06))                       #实体柱
            is_bdouble_red_head = is_bdouble_red_head & (min(upper_shadow, lower_shadow) > column_shadow) #双头线

            #吊脚线
            is_t = True
            is_t = is_t & (last2_is_red)                                #前天收红
            is_t = is_t & (last1_stock_day.close > 0.998 * last1_stock_day.op)          #收红或小阴
            is_t = is_t & (last1_upper_shadow > 2 * last1_column_shadow)                        #昨天倒T
            is_t = is_t & ((lower_shadow > 0.018) & (column_shadow > 0.006))

            #小双头大阳线:双头线越短越适宜
            is_sdouble_red_head = True
            is_sdouble_red_head = is_sdouble_red_head & (is_red)
...
#	    print 'today = ' + TimeUtils.timestamp2datestring(self.today) + '-' + str(is_hit)
            ...
            if is_green_rt & is_hit:
		return BaseStockUtils.compose_hit_data(realtime_stock_day, 'CloseGreenRT')

            if is_bdouble_red_head & is_hit:
		return BaseStockUtils.compose_hit_data(realtime_stock_day, 'CloseBRedHead')
	    
            if is_t & is_hit:
		return BaseStockUtils.compose_hit_data(realtime_stock_day, 'CloseT')

            if is_sdouble_red_head & is_hit:
...
if __name__ == '__main__':
    ...
    model_close = ModelClose(hist_symbols_days, latest_days[0].day)
    today = latest_days[0]
