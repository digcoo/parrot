...
class ModelMAScatter:
	...
	try:
	    ...
	    is_hit = is_hit & (realtime_stock_day.close > IndicatorUtils.Lowest_MA(hist_days, TimeUtils.date_add(self.todaystamp,1)))    		#昨日收盘价高于最低ma线
	    is_hit = is_hit & (realtime_stock_day.close < IndicatorUtils.MAX_MA(hist_days, self.todaystamp))			#昨日收盘价低于最高ma线
